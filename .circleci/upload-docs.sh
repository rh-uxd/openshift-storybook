...
if [ -z "${ALREADY_DEPLOYED}" ] # Leave a Github comment
then
  ...
  curl -H "Authorization: token ${GH_TOKEN}" --request POST ${GITHUB_PR_COMMENTS} --data '{"body":"PatternFly-Next preview: '${DEPLOY_DOMAIN}'"}'
else
